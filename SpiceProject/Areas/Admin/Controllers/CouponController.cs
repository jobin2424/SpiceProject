...
namespace SpiceProject.Areas.Admin.Views.Category
{
    [Area("Admin")]
    public class CouponController : Controller
    {
        ...
        public async Task<IActionResult> Create(Coupon coupons)
        {
            ...
            {
                var files = HttpContext.Request.Form.Files; // this is for saving an image 
                if (files.Count > 0)
                {
                    byte[] p1 = null;
                    using (var fs1 = files[0].OpenReadStream())
                    {
                        ...
                        {
                            fs1.CopyTo(ms1);    // this converts our files into a stream of bytes and stores it into a byte
                            p1 = ms1.ToArray();
                        }
                    }
                    coupons.Picture = p1;
